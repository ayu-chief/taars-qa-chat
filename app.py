...
# サイドバー
st.sidebar.title("表示ページを選んでください")
page = st.sidebar.radio("ページ切替", ["類似QA検索チャット", "ジャンル別FAQ一覧"])
...
if page == "類似QA検索チャット":
    ...
    st.markdown("### ジャンルを選んでください")
    genre_list = sorted(df["genre"].dropna().unique())
    selected_genre = st.selectbox("ジャンルを選択", genre_list)
